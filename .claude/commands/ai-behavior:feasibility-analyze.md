...
4. Load schema from `ai_files/schemas/feasibility_analysis_schema.json` (if exists) or use the schema definition from `schemas/feasibility_analysis_schema.json`
...
This command does NOT use subagents. All steps are executed directly by the main agent to preserve full conversational context and maintain the expert consultant persona throughout the session.
